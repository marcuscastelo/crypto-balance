...
use crate::constants::MATIC_IN_WEI;
...
pub struct Polygonscan;

impl BlockExplorer for Polygonscan {
    fn fetch_native_balance(&self, evm_address: &str) -> TokenBalance {
        let api_key = &CONFIG.polygonscan_api_key;
        let url = format!(
        ...
        let balance = resp.result.parse::<f64>().unwrap() / (MATIC_IN_WEI as f64);
    ...
    fn get_native_token(&self) -> Token {
        Token::Native(NativeTokenName::MATIC)
    }
}
